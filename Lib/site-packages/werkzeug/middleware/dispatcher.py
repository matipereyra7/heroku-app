...
This example dispatches to an API api under "/api", an admin api
under "/admin", and an api that serves frontend files for all other
requests::

    api = DispatcherMiddleware(serve_frontend, {
        '/api': api_app,
        '/admin': admin_app,
    })
